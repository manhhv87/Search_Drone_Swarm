...
target = vec2(random.uniform(0, SCREEN_WIDTH/2),
              random.uniform(0, SCREEN_HEIGHT/2))

simulation = Simulation(screenSimulation, RateSimulation(
    10, [5, 10, 15], [10, 15, 30], [DefineTargetScan(), RowScan()]))
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if pygame.mouse.get_pressed()[0] == True:
                target = vec2(pygame.mouse.get_pos()[
    ...
    for idx, time in enumerate(simulation.rate.out_time_mission):
        try:
            ...
            img = screenSimulation.font20.render(
                f'{idx+1} - Search: {search} -  Scan Time: {time:.2f}, {simulation.rate.print_simulation_idx(idx)}', True, LIGHT_BLUE)
        except:
            search = simulation.rate.in_algorithms[idx].to_string()
            img = screenSimulation.font16.render(
                f'{idx+1} -  Search: {search} - Scan Time: {time}', True, LIGHT_BLUE)
        screenSimulation.screen.blit(img, (20, 20*(idx+2)))

    # Writes the App name in screen
    img = screenSimulation.font24.render(
