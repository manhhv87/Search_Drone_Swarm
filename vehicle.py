...
class Vehicle(object):
    ...
    def __init__(self, x, y, behavior, window):
        ...
        # inicital rotation
        self.rotation = atan2(self.location.y, self.location.x)
    ...
    def stay_at(self, center, r=RADIUS_TARGET):
        ...
        if posToCenter.length() > r:
            ...
            # ok
            pg.draw.line(self.window, (0, 0, 255), center,
                         center+centerToPerimeter, 5)
            ...
            if self.debug == True:
                pg.draw.line(self.window, (0, 255, 0), center,
                             new_target, 5)  # verde é o target
    ...
    def draw(self, window):
        ...
        if len(self.memory_location) >= 2:
            pg.draw.lines(self.window, self.color_target,
                          False, self.memory_location, 1)
        # Drawing drone's outer circle as a hitbox?
        if self.debug == True:
            pg.draw.circle(self.window, (100, 100, 100),
                           self.location, AVOID_DISTANCE, 1)
    ...
    def collision_avoidance(self, positions_drones, pos_obstacles, index):
        ...
        for p in positions_drones:
            ...
            if (d < dist_avoid) and (aux != index):
                f_repulsion = derivativeBivariate(
                    0.001, .001, p.location, self.location)/SAMPLE_TIME
                # print(f_repulsion)
        ...
        for p in pos_obstacles:
            ...
            if (d < dist_avoid):
                f_repulsion = derivativeBivariate(
                    factor_repulsion, factor_repulsion, p, self.location)/SAMPLE_TIME
                f_repulsion = limit(f_repulsion, self.max_force*1.8)
                self.applyForce(-f_repulsion)
             # ----
