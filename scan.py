...
class ScanInterface:
    ...
    def draw_legend(self, drone, simulation, index):
        ...
        # writes drone id
        img = simulation.screenSimulation.font20.render(
            f'Drone {index+1}', True, LIGHT_BLUE)
        simulation.screenSimulation.screen.blit(
            img, drone.get_position()+(0, 20))
        # writes drone current behavior
        img = simulation.screenSimulation.font16.render(
            drone.behavior.get_current_state(), True, LIGHT_BLUE)
        simulation.screenSimulation.screen.blit(
            img, drone.get_position()+(0, 30))
        # writes drone current position in grid
        p = drone.get_position()
        col = int(p.x/RESOLUTION)
        row = int(p.y/RESOLUTION)
        img = simulation.screenSimulation.font16.render(
            f'Pos:{col},{row}', True, LIGHT_BLUE)
        simulation.screenSimulation.screen.blit(
            img, drone.get_position()+(0, 40))
