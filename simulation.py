...
class RateSimulation(object):
    def __init__(self, in_repetitions, in_num_swarm, in_num_obstacles, in_algorithms):
        ...
        self.in_repetitions = in_repetitions * \
            len(in_num_swarm) * len(in_num_obstacles) * len(in_algorithms)
    ...
    def print_plan_rate(self):
        for idx in range(0, self.in_repetitions):
            print(
                f'{idx+1} - num_obstacles: {self.in_num_obstacles[idx]}, num_swarm : {self.in_num_swarm[idx]}, algorithm : {self.in_algorithms[idx].to_string()},')
    ...
    def print_rate(self):
        ...
        for idx in range(0, len(self.out_time_target)):
            print(
                f'{idx+1} - Time Target: {self.out_time_target[idx]}, Time Mission: {self.out_time_mission[idx]}, num_uav: {self.out_num_uav[idx]}, num_swarm: {self.in_num_swarm[idx]}, num_obstacles: {self.in_num_obstacles[idx]}')
    ...
    def save_csv(self):
        with open('result.csv', 'w', newline='') as file:
            ...
            writer.writerow(["Execution", "N Drones", "N Obstacles",
                            "Algorithm", "Time Found Target", "Time Mission Completed"])
            ...
            for idx in range(0, self.in_repetitions):
                ...
                writer.writerow([idx+1, self.in_num_swarm[idx], self.in_num_obstacles[idx],
                                self.in_algorithms[idx].to_string(), self.out_time_target[idx], self.out_time_mission[idx]])
...
class Simulation(object):
    ...
    def __init__(self, screenSimulation, rate: RateSimulation):
        ...
        self.obstacles = Obstacles(
            rate.in_num_obstacles[0], (SCREEN_WIDTH, SCREEN_HEIGHT))
        self.list_obst = []
    ...
    def create_swarm_uav(self, num_swarm, search_pattern='DefineTargetScan'):
        ...
        for d in range(0, num_swarm):
            ...
            if search_pattern == 'DefineTargetScan':
                self.behaviors.append(FiniteStateMachine(
                    SeekState()))  # Inicial state
            else:
                self.behaviors.append(FiniteStateMachine(
                    SearchTargetState()))  # Inicial state

            drone = Vehicle(uniform(0, 100), uniform(0, 100),
                            self.behaviors[-1], self.screenSimulation.screen)
            self.swarm.append(drone)

    def add_new_uav(self):
        self.behaviors.append(FiniteStateMachine(SeekState()))
        drone = Vehicle(SCREEN_WIDTH/2, SCREEN_HEIGHT/2,
                        self.behaviors[-1], self.screenSimulation.screen)

        drone.set_target(vec2(pygame.mouse.get_pos()[
    ...
    def draw_obstacles(self):
        ...
        for _ in self.list_obst:
            ...
            pygame.draw.circle(self.screenSimulation.screen,
                               (200, 200, 200), _, radius=RADIUS_OBSTACLES, width=1)
            pygame.draw.circle(self.screenSimulation.screen, (200, 200, 200),
                               _, radius=RADIUS_OBSTACLES*1.6 + AVOID_DISTANCE, width=1)
    ...
    def run_simulation(self):
        ...
        # count time of execution based on the sampling
    ...
    def generate_new_random_target(self):
        ...
        while not found_valid_target:
            # generates new point
            target = vec2(uniform(SCREEN_WIDTH/3, SCREEN_WIDTH),
                          uniform(100, SCREEN_HEIGHT))
            c = 0
    ...
    def rest_simulation(self):
        ...
        self.create_swarm_uav(
            self.rate.in_num_swarm[self.rate.current_repetition], serch_patter_for_iteration)
        ...
        self.rate.in_algorithms[self.rate.current_repetition].prepare_simulation(
            self, target)
        self.found = False
